...
#         assert self.action_space.contains(action), err_msg
        ...
        #         force=(1-self.smooth)*self.force+self.smooth*force #remove (1-self.smooth) ??
        ...
        #baselines 1: a=0.05; b=f=g=e=0; c=1; d=1; no smooth
        #baselines 2: a=0.05; b=f=e=0; g=0.0001; c=1; d=1; no smooth
        #baselines 3: a=0.05; b=f=e=g=0; h=2.0; c=1; d=1.5; no smooth
        #baselines 4: a=0.05; b=f=e=g=0; e=2.0; c=1; d=2.0; smooth adap
        #baselines 5: a=0.5; b=0.02; c=1.0; d=0.01; e=0.05; smooth adap
        #baselines 6: a=0.05; b=100.0; c=1.0; d=0.25; rest=0; smooth const 0.75 [X]
        #adap/learned sigma: a=0.05; b=10; c=1; d=0.25; smooth
        a=0.05
        b=0. #10.0 #0.02 #1000.0 #0.2
        c=1.0
        d=1. #0.25 #1.0 #0.2
        e=0.0#5 #3.0 #18.0#*self.T #0.72
        f=0.#1 #0.0001 #0.1
        g=0.000#1
        h=0.0 #3.0
        P=np.array([
        ...
        if not done:
            ...
            # reward=1-(2*np.abs(np.sin(theta)))-(0.1*theta_dot**2)-(0.002*force**2)#-(0.1*x_dot**2)
            ...
            if (np.abs(x)>=self.x_threshold): P[0,0]=-100#*a
            reward=0.5*ds@P@ds.T-e*force**2-f*delta**2-g*thetaacc**2-h*diff**2
            
            # if (np.abs(x)>=self.x_threshold): w=100
            # reward=1-(2*np.abs(np.sin(theta)))-(0.1*theta_dot**2)-(0.002*force**2)#-(0.1*x_dot**2)-(w*np.abs(x))
