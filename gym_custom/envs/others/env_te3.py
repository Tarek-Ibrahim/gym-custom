...
#         assert self.action_space.contains(action), err_msg
        ...
        # force= np.sign(force)*self.force_mag if np.abs(force)>self.force_mag else force #saturate
        ...
        if not done:
            # thr=np.array([self.x_threshold,self.theta_threshold_radians])
            # real=np.array([x,theta])
            reward = 1.0 #0.5*((real-thr)@(real-thr).T) + 1/(force+self.eps) + 1/(delta+self.eps) #1.0
