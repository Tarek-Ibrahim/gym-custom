...
class LunarLanderRandomized(gym.Env, EzPickle):
    ...
    def step(self, action): #action=[main power, side power]
        if self.continuous:
        ...
        state = [
            (pos.x - VIEWPORT_W / SCALE / 2) / (VIEWPORT_W / SCALE / 2), # x pos
            (pos.y - (self.helipad_y + LEG_DOWN / SCALE)) / (VIEWPORT_H / SCALE / 2), #y pos
            vel.x * (VIEWPORT_W / SCALE / 2) / FPS, #x vel
            vel.y * (VIEWPORT_H / SCALE / 2) / FPS, #y vel
            self.lander.angle, #angular position
            20.0 * self.lander.angularVelocity / FPS, #angular velocity
            1.0 if self.legs[0].ground_contact else 0.0, #whether left leg is in contact with the ground
            1.0 if self.legs[1].ground_contact else 0.0 #whether right leg is in contact with the ground
        ]
        assert len(state) == 8
